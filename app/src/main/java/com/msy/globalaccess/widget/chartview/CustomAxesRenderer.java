 ...
 */
public class CustomAxesRenderer {
    ...
    private static final int labelSize = 5;
    ...
    private Paint.FontMetricsInt[] fontMetricsTab = new Paint.FontMetricsInt[]{new Paint.FontMetricsInt(), new Paint
            .FontMetricsInt(),
    ...
    public CustomAxesRenderer(Context context, CustomChart chart) {
        this.chart = chart;
    ...
    //绘制多行X轴,每行最多显示5个字,最多2行,第一行如果有换行符换行导致字数不满5个,则居中
    private void drawMultiLineText(String text, Axis axis, float labelX, float labelY, int position, Canvas canvas) {
        float labelInterval = 35f + axis.getTextSize();
        String[] labels = new String[2];
        int index = text.indexOf("\n");

        if (index >= 0 && index < labelSize) {
            labels = text.split("\n");
        } else if (index == -1) {
            labels[0] = text.substring(0, labelSize);
            labels[1] = text.substring(labelSize, text.length());
        } else {
            String temp = text.replace("\n", "");
            labels[0] = temp.substring(0, labelSize);
            labels[1] = temp.substring(labelSize, temp.length());
        }

        if (axis instanceof CustomAxis&&!((CustomAxis) axis).isLimitLength()) {

        } else if (labels[1].length() > labelSize) {
            labels[1] = labels[1].substring(0, labelSize - 1) + "..";
        }

        canvas.drawText(labels[0], labelX, labelY, labelPaintTab[position]);
        canvas.drawText(labels[1], labelX, labelY + labelInterval, labelPaintTab[position]);
    }
    ...
    private void drawAxisLabelsAndName(Canvas canvas, Axis axis, int position) {
        ...
        for (int valueToDrawIndex = 0; valueToDrawIndex < valuesToDrawNumTab[position]; ++valueToDrawIndex) {
            ...
            if (axis.hasTiltedLabels()) {
                ...
                text = text.substring(text.length() - charsNumber, text.length());
                text = text.replace("\u0000", "");

                if (text.contains("\n") || text.length() > labelSize) {
                    drawMultiLineText(text, axis, labelX, labelY, position, canvas);
                } else {
                    canvas.drawText(labelBuffer, labelBuffer.length - charsNumber, charsNumber, labelX, labelY,
                            labelPaintTab[position]);
                }
                ...
                //                if (text.length() > 5) {
                //                    String[] labels = new String[2];
                //                    float labelHeight = axis.getTextSize();
                //                    float labelInterval = 35f;
                //                    labels[0] = text.substring(0, 5);
                //                    labels[1] = text.substring(5, text.length());
                //                    if (labels[1].length() > 5) {
                //                        labels[1] = labels[1].substring(0, 4) + "..";
                //                    }
                //                    canvas.drawText(labels[0], labelX, labelY, labelPaintTab[position]);
                //                    canvas.drawText(labels[1], labelX, labelY + labelHeight + labelInterval,
                //                            labelPaintTab[position]);
                //                }
            }
        }
